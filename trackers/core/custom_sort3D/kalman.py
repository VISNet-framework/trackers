...
class CustomSORT3DKalmanBoxTracker:
    """
    ...
    B: NDArray[np.float32] | None = None ## control transition matrix
    ...
    def __init__(self, xyzxyz: NDArray[np.float64]) -> None:
        ...
        # (x, y, z, x2, y2, z2, vx, vy, vz, vx2, vy2, vz2).
        # We'll store the bounding box in "self.state"
        self.state = np.zeros((12, 1), dtype=np.float32)

        # Initialize state directly from the first detection
        xyzxyz_float: NDArray[np.float32] = xyzxyz.astype(np.float32)
        self.state[:6,0] = xyzxyz_float
    ...
    def _initialize_kalman_filter(self) -> None:
        ...
        # velocity each step.
        self.F = np.eye(12, dtype=np.float32)
        for i in range(6):
            self.F[i, i + 6] = 1.0

        # Measurement matrix (H): we directly measure x1, y1, x2, y2
        self.H = np.eye(6, 12, dtype=np.float32)  # 4x8

        # Process covariance matrix (Q)
        self.Q = np.eye(12, dtype=np.float32) * 0.01 #8

        # Measurement covariance (R): noise in detection
        self.R = np.eye(6, dtype=np.float32) * 0.1 #4

        # Error covariance matrix (P)
        self.P = np.eye(12, dtype=np.float32) # 8

        ## input matrix
        self.B = np.zeros((self.F.shape[0],6), dtype=np.float32)
        self.B[0,0] = 1
        self.B[1,1] = 1
        self.B[2,2] = 1
        self.B[3,3] = 1
    ...
    def update(self, bbox: NDArray[np.float64]) -> None:
        ...
        # Residual
        measurement: NDArray[np.float32] = bbox.reshape((6, 1)).astype(np.float32)
        ...
        # Update covariance
        identity_matrix: NDArray[np.float32] = np.eye(12, dtype=np.float32)
    ...
    def get_state_bbox(self) -> NDArray[np.float32]:
        ...
        """
        return self.state[:6, 0].flatten().astype(np.float32)
