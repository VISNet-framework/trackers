...
from trackers.core.custom_sort.kalman import CustomSORTKalmanBoxTracker
...
class CustomSORTTracker(BaseTracker):
    ...
    def __init__(
        ...
        # Active trackers
        self.trackers: list[CustomSORTKalmanBoxTracker] = []
        ...
        for detection_idx in unmatched_detections:
            if (
                ...
                self.trackers.append(
                    CustomSORTKalmanBoxTracker(detection_boxes[detection_idx])
                )

    def update(self, detections: sv.Detections, u: np.ndarray | None) -> sv.Detections:
        ...
        matched_tracker_for_det: dict[int, CustomSORTKalmanBoxTracker] = {}
        ...
        for det_idx, tracker in matched_tracker_for_det.items():
            if tracker.number_of_successful_updates >= self.minimum_consecutive_frames:
                if tracker.tracker_id == -1:
                    tracker.tracker_id = CustomSORTKalmanBoxTracker.get_next_tracker_id()
    ...
    def reset(self) -> None:
        ...
        """
        self.trackers = []
        CustomSORTKalmanBoxTracker.count_id = 0
